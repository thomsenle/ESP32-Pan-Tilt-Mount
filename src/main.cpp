...
const char* ssid = "boingboing";
const char* password = "wirdarfen";
...
int8_t stallvalue = 100;


byte acceleration_enable_state = 0;

TMC2209Stepper SLdriver = TMC2209Stepper(&SERIAL_PORT, R_SENSE,SL_DRIVER_ADDRESS); // Hardware Serial2
TMC2209Stepper PANdriver = TMC2209Stepper(&SERIAL_PORT, R_SENSE,PAN_DRIVER_ADDRESS); // Hardware Serial2
TMC2209Stepper TILTdriver = TMC2209Stepper(&SERIAL_PORT, R_SENSE,TILT_DRIVER_ADDRESS); // Hardware Serial2

AccelStepper stepper_slider = AccelStepper(AccelStepper::DRIVER, PIN_STEP_SLIDER, PIN_DIRECTION_SLIDER);
AccelStepper stepper_pan = AccelStepper(AccelStepper::DRIVER, PIN_STEP_PAN, PIN_DIRECTION_PAN);
AccelStepper stepper_tilt = AccelStepper(AccelStepper::DRIVER, PIN_STEP_TILT, PIN_DIRECTION_TILT);
...
// Variables to store Buttonstate
int8_t DpadUp=0;
int8_t DpadDown = 0;
int8_t DpadLeft = 0; 
int8_t DpadRight = 0;
...
int16_t RXj = 0;
int16_t RYj = 0;
int8_t ButtonCross = 0;
int8_t ButtonTriangle = 0;
int8_t ButtonCircle = 0;
int8_t ButtonSquare = 0;
int8_t ShoulderLeft = 0;
int8_t ShoulderRight = 0;
int8_t TriggerLeft = 0;
int8_t TriggerRight = 0;
int8_t ButtonSelect = 0;
int8_t ButtonStart = 0;
int8_t ButtonPS = 0;

FloatCoordinate intercept;
int battery_controler = 0;
int battery_max_voltage = 24;
int battery_min_voltage = 10;
...
float getBatteryVoltage(void){ //TODO: Calibrate the values for your battery
    return (float)analogRead(PIN_INPUT_VOLTAGE) / 4096 * battery_max_voltage * (159442 / 16000);
    //return 1000;
}
...
float getBatteryPercentage(void){ //TODO: Calibrate the values for your battery
    return boundFloat(mapNumber(getBatteryVoltage(), battery_min_voltage, battery_max_voltage, 0, 100), 0, 100); //780 = 9V = 0%, 1023 = 12.6V = 100%
}
...
void move(){
    //Serial.println("Move");
	float magnitude = sqrt(RXj * RXj + RYj * RYj);
	...
	if(magnitude > INPUT_DEADZONE){ //check if the controller is outside a circular dead zone
		...
		RXShort = -((RXj * scale) * abs(RXj * scale) * MAXIMUM_PAN_STEP_SPEED);
		RYShort = -((RYj * scale) * abs(RYj * scale) * MAXIMUM_TILT_STEP_SPEED);
	}
	else{
		RXShort = 0;
		RYShort = 0;
    ...
    stepper_slider.setSpeed(LXShort);
    stepper_pan.setSpeed(RXShort);
    stepper_tilt.setSpeed(RYShort);
    stepper_slider.runSpeed();
    stepper_pan.runSpeed();
    stepper_tilt.runSpeed();
}

void setStepMode(int newMode){ //Step modes for the TMC2209
    ...
    if(newMode == HALF_STEP){
		...
		SLdriver.microsteps(2);
		PANdriver.microsteps(2);
		TILTdriver.microsteps(2);
    }
    else if(newMode == QUARTER_STEP){
        //PORTB |=   B00000100; //MS2 high
        //PORTB &= ~(B00001000); //MS1 low
		SLdriver.microsteps(4);
		PANdriver.microsteps(4);
		TILTdriver.microsteps(4);
    }
    else if(newMode == EIGHTH_STEP){
        //PORTB &= ~(B00001100); //MS1 and MS2 low
		SLdriver.microsteps(8);
		PANdriver.microsteps(8);
		TILTdriver.microsteps(8);
    }
    else if(newMode == SIXTEENTH_STEP){
        //PORTB |= B00001100; //MS1 and MS2 high
		SLdriver.microsteps(16);
		PANdriver.microsteps(16);
		TILTdriver.microsteps(16);
    }
...
void printEEPROM(void){
    ...
    printi(F("EEPROM:\nStep mode: "), preferences.getInt("MODE", 0), F("\n"));
    //EEPROM.get(EEPROM_ADDRESS_PAN_MAX_SPEED, ftemp);
    printi(F("Pan max: "), preferences.getFloat("P_MAX_SPEED", 0), 3, F("º/s\n"));
    //EEPROM.get(EEPROM_ADDRESS_TILT_MAX_SPEED, ftemp);
    printi(F("Tilt max: "), preferences.getFloat("T_MAX_SPEED", 0), 3, F("º/s\n"));
    //EEPROM.get(EEPROM_ADDRESS_SLIDER_MAX_SPEED, ftemp);
    printi(F("Slider max: "), preferences.getFloat("S_MAX_SPEED", 0), 3, F("mm/s\n")); 
    //EEPROM.get(EEPROM_ADDRESS_HALL_PAN_OFFSET, ftemp);
    printi(F("Pan offset: "), preferences.getFloat("H_PAN_OFFSET", 0), 3, F("º\n"));
    //EEPROM.get(EEPROM_ADDRESS_HALL_TILT_OFFSET, ftemp);
    printi(F("Tilt offset: "), preferences.getFloat("H_TILT_OFFSET", 0), 3, F("º\n"));
    //EEPROM.get(EEPROM_ADDRESS_DEGREES_PER_PICTURE, ftemp);
    printi(F("Angle between pics: "), preferences.getFloat("D_PER_PICTUR", 0), 3, F(" º\n"));
    //EEPROM.get(EEPROM_ADDRESS_PANORAMICLAPSE_DELAY, ltemp);
    printi(F("Delay between pics: "), preferences.getULong("PANO_DELAY", 0), F("ms\n"));   
    printi(F("Pan invert: "), preferences.getInt("INVERT_PAN", 0));
    printi(F("Tilt invert: "), preferences.getInt("INVERT_TILT", 0));
    printi(F("Slider invert: "), preferences.getInt("INVERT_SLIDER",0)); 
    printi(F("Homing mode: "), preferences.getInt("HOMING_MODE",0));
    printi(F("Accel enable: "), preferences.getInt("ACCEL_ENABLE",0));
    //EEPROM.get(EEPROM_ADDRESS_PAN_ACCEL_INCREMENT_DELAY, itemp);
    printi(F("Pan accel delay: "), preferences.getInt("P_AC_I_DELAY",0), F("us\n"));
    //EEPROM.get(EEPROM_ADDRESS_TILT_ACCEL_INCREMENT_DELAY, itemp);
    printi(F("Tilt accel delay: "), preferences.getInt("T_A_I_DELAY", 0), F("us\n"));
    //EEPROM.get(EEPROM_ADDRESS_SLIDER_ACCEL_INCREMENT_DELAY, itemp);
    printi(F("Slider accel delay: "), preferences.getInt("S_A_I_DELAY", 0), F("us\n"));
    preferences.end();  
}
...
void serialDataBT(void){
    char instruction = SerialBT.read();
    if(instruction == INSTRUCTION_BYTES_SLIDER_PAN_TILT_SPEED){
        int count = 0;
        while(SerialBT.available() < 6){//Wait for 6 bytes to be available. Breaks after ~20ms if bytes are not received.
            ...
            int sliderStepSpeed = (SerialBT.read() << 8) + SerialBT.read(); 
            int panStepSpeed = (SerialBT.read() << 8) + SerialBT.read(); 
            int tiltStepSpeed = (SerialBT.read() << 8) + SerialBT.read(); 
    ...
    while(SerialBT.available()){//set elemetns of stringText to the serial values sent
        char digit = SerialBT.read(); //read in a char
...
*/
   ...
   if( abs(Ps3.event.analog_changed.stick.lx) + abs(Ps3.event.analog_changed.stick.ly) > 2 ){
	   ...
	   LX= 256*Ps3.data.analog.stick.lx;
	   LY= 256*Ps3.data.analog.stick.ly;
       //Serial.println();
    }
   ...
   if( abs(Ps3.event.analog_changed.stick.rx) + abs(Ps3.event.analog_changed.stick.ry) > 2 ){
     ...
     //  Serial.print(" y="); Serial.print(Ps3.data.analog.stick.ry, DEC);
	   RXj= 256*Ps3.data.analog.stick.rx;
	   RYj= 256*Ps3.data.analog.stick.ry;
     //  Serial.println();
   }

/* Not used --------------- Analog D-pad button events ----------------
...
Not used ---------- Analog shoulder/trigger button events ----------
...
*/
   ...
   //---------------------- Battery events ---------------------
    if( battery_controler != Ps3.data.status.battery ){
        battery_controler = Ps3.data.status.battery;
        Serial.print("The controller battery is ");
        if( battery_controler == ps3_status_battery_charging )      Serial.println("charging");
        else if( battery_controler == ps3_status_battery_full )     Serial.println("FULL");
        else if( battery_controler == ps3_status_battery_high )     Serial.println("HIGH");
        else if( battery_controler == ps3_status_battery_low)       Serial.println("LOW");
        else if( battery_controler == ps3_status_battery_dying )    Serial.println("DYING");
        else if( battery_controler == ps3_status_battery_shutdown ) Serial.println("SHUTDOWN");
...
/*
    ...
    adcAttachPin(PIN_PAN_HALL);
    pinMode(PIN_TILT_HALL,INPUT);
    adcAttachPin(PIN_TILT_HALL);
    pinMode(PIN_SLIDER_HALL,INPUT);
    adcAttachPin(PIN_SLIDER_HALL);
...
*/
  ...
  if(!SerialBT.begin("Slider")){
    Serial.println("An error occurred initializing Bluetooth");
  }else{
    Serial.println("Bluetooth COM initialized");
  }
...
if(!display.begin(SSD1306_SWITCHCAPVCC, 0x3C)) { // Address 0x3D for 128x64
  ...
  display.display(); 
	...
	SLdriver.begin();             // Initiate pins and registeries
    SLdriver.rms_current(600);    // Set stepper current to 600mA. The command is the same as command TMC2130.setCurrent(600, 0.11, 0.5);
    SLdriver.microsteps(16);

	PANdriver.begin();             // Initiate pins and registeries
    PANdriver.rms_current(600);    // Set stepper current to 600mA. The command is the same as command TMC2130.setCurrent(600, 0.11, 0.5);
    PANdriver.microsteps(16);
	
	TILTdriver.begin();             // Initiate pins and registeries
    TILTdriver.rms_current(600);    // Set stepper current to 600mA. The command is the same as command TMC2130.setCurrent(600, 0.11, 0.5);
    TILTdriver.microsteps(16);
...
void drinti(String text, float num, int dp, String endText) {
	display.print(text);
	display.print(num, dp);
	display.println(endText);
}
...
void loop()
{
  ...
  if (SerialBT.available()) serialDataBT();
 // if (Serial.available()) serialData();
   ...
   multi_stepper.run();
    ...
    display.setCursor(0, 10);
    //Display static text 
    drinti(F("Pan angle: "), panStepsToDegrees(stepper_pan.currentPosition()), 3, F(""));
    drinti(F("Tilt angle: "), tiltStepsToDegrees(stepper_tilt.currentPosition()), 3, F("")); 
    drinti(F("Slider pos "), sliderStepsToMillimetres(stepper_slider.currentPosition()), 3, F("mm\n")); 
    display.display(); 
}